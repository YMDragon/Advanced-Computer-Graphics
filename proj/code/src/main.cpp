...
void printVector3f(Vector3f v) { printf("%f %f %f\n", v.x(), v.y(), v.z()); }

Vector3f RayTracer(const Ray r, int depth, Vector3f weight, mt19937 *rd)
{
    Hit hit;
    bool isIntersect = sceneparser->getGroup()->intersect(r, hit, 1e-6);
    if (!isIntersect)
        return sceneparser->getBackgroundColor();

    Vector3f hitPosition = r.pointAtParameter(hit.getT());
    Material *material = hit.getMaterial();
    Vector3f color = material->getColor(hit.getTexture());

    if (depth >= 10)
    {
        //double k = max(max(color.x(), color.y()), color.z());
        double k = max(max(weight.x(), weight.y()), weight.z());
        if ((randdouble(rd) < k) && (depth <= 100))
            weight = weight / k;
        else
    ...
    if (material->getType() == DIFF)
    {
        ...
        return weight * material->getEmission() + RayTracer(Ray(hitPosition, R), depth + 1, color * weight, rd);
    }
    ...
    {
        double rate = into ? 1 / material->getRate() : material->getRate();
        double cosThetaI = (into ? -1 : 1) * Vector3f::dot(hit.getNormal(), r.getDirection());
        Vector3f R1 = r.getDirection() - 2 * Vector3f::dot(hit.getNormal(), r.getDirection()) * hit.getNormal();
        if ((1 - cosThetaI * cosThetaI) * rate * rate > 1)
            return weight * material->getEmission() + RayTracer(Ray(hitPosition, R1), depth + 1, color * weight, rd);
        double cosThetaO = sqrt(1 - rate * rate * (1 - cosThetaI * cosThetaI));
        Vector3f R2 = rate * r.getDirection() - (rate * Vector3f::dot(hit.getNormal(), r.getDirection()) + (into ? 1 : -1) * cosThetaO) * hit.getNormal();
        double r0 = (rate - 1) * (rate - 1) / (rate + 1) / (rate + 1);
        double r = r0 + (1 - r0) * pow(1 - (into ? cosThetaI : cosThetaO), 5);
        if (depth < 3)
            return weight * material->getEmission() + RayTracer(Ray(hitPosition, R1), depth + 1, color * r * weight, rd) + RayTracer(Ray(hitPosition, R2), depth + 1, color * (1 - r) * weight, rd);
        else
        {
            if (randdouble(rd) < r)
                return weight * material->getEmission() + RayTracer(Ray(hitPosition, R1), depth + 1, color * weight, rd);
            else
                return weight * material->getEmission() + RayTracer(Ray(hitPosition, R2), depth + 1, color * weight, rd);
        }
    }
    else if (material->getType() == MIX)
    {
        Vector3f R;
        if (randdouble(rd) < material->getRate())
            R = cosineWeightSample((into ? 1 : -1) * hit.getNormal(), rd);
        else
            R = r.getDirection() - 2 * Vector3f::dot(hit.getNormal(), r.getDirection()) * hit.getNormal();
        return weight * material->getEmission() + RayTracer(Ray(hitPosition, R), depth + 1, color * weight, rd);
    }
    else
    {
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int x = 0; x < camera->getWidth(); x++)
    {
        ...
        for (int y = 0; y < camera->getHeight(); y++)
        {
            ...
            for (int i = 0; i < M; i++)
            {
                Ray camRay = camera->generateRay(Vector2f(x + randdouble(&rd), y + randdouble(&rd)), &rd);
